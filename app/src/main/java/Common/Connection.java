            ...
            for(int c=0; c < Count.length; c++)
            {
                if(c==0)
                {
                    if (cur_H.getString(c) == null)
                        ...
                        DataList = cur_H.getString(c).toString().replace("^","").replace("'","");
...
*/
    ...
    */
    ...
    //Download
    //**********************************************************************************************
    public void Sync_Download_Batch(String TableName, String UserId, String WhereClause) {
        try {
            //Retrieve sync parameter
            //------------------------------------------------------------------------------------------
            String[] SyncParam = Sync_Parameter(TableName);
    ...
    public void Sync_Download_Batch_Migration(String TableName, String UserId, String WhereClause) {
        try {
            //Retrieve sync parameter
            //------------------------------------------------------------------------------------------
            String[] SyncParam = Sync_Parameter(TableName);
            ...
            for (int i = 0; i < totalBatch; i++) {
                ...
                Res = DownloadJSON_Batch_Migration(SQL, TableName, VariableList, UniqueField, UserId);
            }
        }catch (Exception ex)
        {
    ...
    private String DownloadJSON_Batch_Migration(String SQL, String TableName, String ColumnList, String UniqueField, String UserId) {
        String WhereClause = "";
        int varPos = 0;
        int varPos_modifyDate = 0;
