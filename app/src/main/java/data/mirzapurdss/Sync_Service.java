...
public class Sync_Service extends Service {
    ...
    private void handleIntent(Intent intent) {
        ...
        new DataSyncTask().execute(MySharedPreferences.getValue(this,"cluster")+'-'+MySharedPreferences.getValue(this,"rnd"));
    }
    ...
    private class DataSyncTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            final String Cluster = params[0].split("-")[0];
            final String Rnd = params[0].split("-")[1];
            try {
                new Thread() {
                    public void run() {
                        try {
                            Common.Connection.SyncDataService(Cluster,Rnd);
                        } catch (Exception e) {
