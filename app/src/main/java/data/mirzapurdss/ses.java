...
public class ses extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ses_newform);
					...
					try {
						...
						if (Global.Left(spnVisit.getSelectedItem().toString(), 1).equals("3") | Global.Left(spnVisit.getSelectedItem().toString(), 1).equals("7")) {
							...
							try {
								if (!available) {
									...
									SQL += " Visit='" + Global.Left(spnVisit.getSelectedItem().toString(), 1) + "', ";
									SQL += " Q015a='',";
									SQL += " Q015b='',";
									SQL += " Q015c='',";
									...
									SQL += " where status='s' and Vill||Bari||HH = '" + Household + "'";
									SQL += " and SESNo='" + spnSESNo.getSelectedItem().toString() + "'";
						...
						if (rdoYQ026.isChecked()) {
							...
							if (txtQ028b.getText().length() == 0) {
								Connection.MessageBox(ses.this, "সদস্যের কতজন আত্মীয় উত্তর আমেরিকায় থাকে তথ্য খালি রাখা যাবে না ");
								return;
							}
							if (txtQ028c.getText().length() == 0) {
								Connection.MessageBox(ses.this, "সদস্যের কতজন আত্মীয় অস্ট্রেলিয়া/নিউজিল্যান্ড থাকে তথ্য খালি রাখা যাবে না ");
								return;
							}
							if (txtQ028d.getText().length() == 0) {
								Connection.MessageBox(ses.this, "সদস্যের কতজন আত্মীয় মধ্যপ্রাচ্য থাকে তথ্য খালি রাখা যাবে না ");
								return;
							}
							if (txtQ028e.getText().length() == 0) {
								Connection.MessageBox(ses.this, "সদস্যের কতজন আত্মীয়  মালয়েশিয়া/সিঙ্গাপুর/ব্রুনাই থাকে তথ্য খালি রাখা যাবে না ");
								return;
							}
		...
		spnVisit.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			@Override
			public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
				if(spnVisit.getSelectedItemPosition()==0)
				{
					ClearForm();
				...
				if(Common.Global.Left(spnVisit.getSelectedItem().toString(),1).equals("1"))
				{
